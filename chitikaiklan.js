...
window.CHITIKA_ADS = window.CHITIKA_ADS ? window.CHITIKA_ADS : (function() {
    ...
    function bridge_amm() {
        ...
        if ((unit.width == 250 && unit.height == 250) || (unit.width == 120 && unit.height == 600)) {
            if ( unit.width == 250 && unit.height == 250 ) { unit.width = 300; }
            if ( unit.width == 120 && unit.height == 600 ) { unit.width = 160; }
        }
...
*/
        ...
        try {
            ...
            //send_event('getads_except_prepare_instructions', null, {message: e.message});
        }
        try {
            prepare_containers();
        } catch (e) {
            //send_event('getads_except_prepare_containers', null, {message: e.message});
        }
        try {
            prepare_ad_urls();
        } catch (e) {
            //send_event('getads_except_prepare_ad_urls', null, {message: e.message});
        }
        try {
            make_ad_server_call();
        } catch (e) {
            //send_event('getads_except_make_ad_server_call', null, {message: e.message});
        }
...
*/
    ...
    function send_event(event_name, impId, metadata) {
        if (event_name == 'imp_visible') {
            //get size of the container parent
            var unit_id = metadata.unit_id;
            var unit = CHITIKA.units[unit_id];
            var d = unit.container_document ? unit.container_document : document;
            ...
            var found_div = 1;
            while (found_div) {
                if (parent_container.tagName == 'DIV' || parent_container.tagName == 'BODY') {
                    found_div = 0;
                } else {
                    parent_container = parent_container.parentNode;
                }
            }
            var container_info = parent_container.getBoundingClientRect();
            var container_width = container_info.width;
            var container_height = container_info.height;
        }
        ...
        if (event_name == 'imp_visible') {
            url = param_concat_escape(url, 'container_height', container_height);
            url = param_concat_escape(url, 'container_width', container_width);
        }
        if (metadata) {
